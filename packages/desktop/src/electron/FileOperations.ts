...
export class FileOperations {
  ...
  constructor(window: BrowserWindow, menu: Menu, userData: DesktopUserData) {
    ...
    ipcMain.on("returnOpenedFile", (event: IpcMainEvent, data: any) => {
      if (data.action === Actions.SAVE_AS) {
        this.saveFile(data.filePath, data.file.fileContent);
        this.window.webContents.send("openFile", {
          file: {
            filePath: data.filePath,
            fileType: extractFileExtension(data.filePath),
            fileContent: data.file.fileContent
          }
        });
      } else if (data.action === Actions.SAVE) {
        if (data.file.filePath === UNSAVED_FILE_NAME) {
          dialog
              ...
              if (!result.canceled) {
                ...
                this.window.webContents.send("openFile", {
                  file: {
                    ...
                    fileType: extractFileExtension(result.filePath!),
                    fileContent: data.file.fileContent
                  }
                });
              }
  ...
  public save() {
    this.window.webContents.send("requestOpenedFile", { action: Actions.SAVE });
  }
